...
const isRequired = value => (value ? undefined : 'Required'); // валидация на пустоту
const isEmail = value => (value.includes('@') ? undefined : 'Should be an email'); // Валидация емейла
...
// const onEmailChange = event => {
//   // Событие на ввод в поле
//   const errorText = [isRequired, isEmail] // Функции валидации кидаем в масив
//     .map(validator => validator(event.target.value)) // проходим мепом, передавая в каждую функцию текст поля
//     .filter(errorText => errorText) // фильтруем, если undefined отбрасивается, если есть текст оставляем
//     .join(','); // склеиваем ошибки запятой
//   emailErrorElem.textContent = errorText;
// };

// const onPasswordChange = event => {
//   // Событие на ввод в поле
//   const errorText = [isRequired] // Функции валидации кидаем в масив
//     .map(validator => validator(event.target.value)) // проходим мепом, передавая в каждую функцию текст поля
//     .filter(errorText => errorText) // фильтруем, если undefined отбрасивается, если есть текст оставляем
//     .join(','); // склеиваем ошибки запятой
//   passwordErrorElem.textContent = errorText;
